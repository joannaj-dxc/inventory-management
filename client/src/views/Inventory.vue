...
export default {
  ...
  setup() {
    ...
    // English status labels used in the CSV regardless of the active locale
    const CSV_STATUS_TEXT = {
      lowStock: "Low Stock",
      adequate: "Adequate",
      inStock: "In Stock",
    };
    ...
    // double any internal double-quote characters per RFC 4180
    ...
    const exportCsv = () => {
      ...
      const rows = filteredItems.value.map((item) => {
        const totalValue = (item.quantity_on_hand * item.unit_cost).toFixed(2);
        const status = CSV_STATUS_TEXT[getStockStatusKey(item)];
        return [
          item.sku,
          item.name,
          item.category,
